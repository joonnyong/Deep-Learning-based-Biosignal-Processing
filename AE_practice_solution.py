...
# 데이터를 가진 파일이 있는 폴더를 data_path 변수에 세팅하시면 됩니다=================================================
...
with tf.Session() as sess:
    ...
    for epoch in range(300):
        ...
        for i in range(total_batch):
            ...
            _, loss1 = sess.run([optimizer, loss], feed_dict={X: batch, Y: label, prob: 0.3, is_training: True})
        ...
        if epoch % 300 == 0:
            zero_data = np.zeros((1, 250), float)
            output_val = sess.run(prediction, feed_dict={X: zero_data})
            figure = plt.figure(figsize=(10, 6))
            plt.plot(output_val[0], color='g')
            plt.xlabel('samples')
            plt.ylabel('PPG')
            plt.savefig('PPG_AE_zero_data_output.png')
